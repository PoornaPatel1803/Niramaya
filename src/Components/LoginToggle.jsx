...
const AuthPage = () => {
  ...
  const handleLogin = () => {
    // Admin login
    if (
      formData.aadhar === adminCredentials.aadhar &&
    ...
    if (user.userType === "doctor" && !user.approved) {
  ...
  const handleSignup = () => {
    ...
    const newUser = {
      ...
      approved: formData.userType === "doctor" ? false : true,
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-indigo-500 to-teal-400 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
      <button
        className="absolute top-4 left-4 flex items-center text-white hover:underline"
        onClick={() => navigate("/")}
      >
